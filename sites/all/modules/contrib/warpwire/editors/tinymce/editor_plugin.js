...
(function($) {
  ...
  tinymce.create('tinymce.plugins.warpwire', {
    init : function(editor) {
      ...
      this.path = '/'+Drupal.settings.warpwire.warpwire_module_path + '/editors/tinymce/';
      ...
      // Add Button
      editor.addButton( 'warpwire', {
        title: 'Warpwire',
        cmd: 'warpwire',
        image: this.path + 'images/icon-warpwire-integration.gif'
      });
        ...
        addEvt(window, "message", function(ev) {
            ...
            if (parsedData.message === "deliverResult") {
              ...
              for(var i=0; i < frames.length; i++) {
                var imgNode  = editor.getDoc().createElement('img');
