...
class OAuthServer {
  ...
  private function get_consumer(&$request) {
    ...
    if (!$consumer_key) {
      throw new OAuthException("The supplied consumer key is not valid.");
    }

    $consumer = $this->data_store->lookup_consumer($consumer_key);
    if (!$consumer) {
      throw new OAuthException("The provided consumer is invalid.");
    }
  ...
  private function check_signature(&$request, $consumer, $token) {
    ...
    if (!$valid_sig) {
      $ex_text = "Invalid signature. Please contact support for assistance.";
      if ( $OAuth_last_computed_signature && $this->_debug) {
  ...
  private function check_timestamp($timestamp) {
    ...
    if ($now - $timestamp > $this->timestamp_threshold) {
      $timestampData = '';
      if ($this->_debug)
      	$timestampData = 'Input: $timestamp Current: $now';
      throw new OAuthException(
        "We are unable to complete this request. The request has expired."
      );
    }
  }
